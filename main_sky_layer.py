...
from skyNet_layer import SkyNet_Layer
...
def preprocess(x):
    src_trgt_layer = tf.string_split([x], delimiter="outputly").values
    ...
    layer = tf.string_to_number(tf.string_split([layer]).values, tf.float32)
    ...
    return src, tf.shape(src)[0], trgt, src_real, trgt_real, tf.shape(src_real)[0], layer
...
def get_iterator(filename, params, is_infer=False):
    ...
    if is_infer:
        dataset = dataset.map(infer_preprocess, num_parallel_calls=8)
        ...
        dataset = dataset.padded_batch(params.batch_size, padded_shapes=(
            [None, params.input_dim], [], [None], [None, params.input_dim], [None], [], [None]),
                                       padding_values=(0.0, 0, 0, 0.0, 0, 0, float('inf')))
...
if __name__ == '__main__':
    ...
    if params.mode == 'train':
        ...
        train_file = "data/skyline10k-1k-2d-layer.txt"
        infer_file = "data/skyline10k-1k-2d-layer.txt"

        with train_graph.as_default(), tf.container("train"):
            train_iter = get_iterator(train_file, params)
            train_op, loss = build_multi_gpu_model(params, SkyNet_Layer, train_iter)
        ...
        with eval_graph.as_default(), tf.container("eval"):
            ...
            eval_model = SkyNet_Layer(params, 'eval', eval_iter.get_next())
        ...
        if params.use_previous_ckpt is True:
        ...
        for i in trange(params.num_epoch):
            ...
            if (i + 1) % params.log_step == 0:
                ...
                test = eval_sess.run({"loss": eval_loss, "true": eval_model.trgt_real, "trgt": eval_model.trgt,
                                      ...
                                      "logits": eval_model.logits, "src": eval_model.src})
                ...
                for j in range(len(test['true'])):
                    ...
                    # tf.logging.info("correct prediction: %d, recall: %.2f, precision: %.2f, f1: %.2f, enclose ratio: %.2f" % (numCorrect, recall, precision, f1, enclose_ratio))
        ...
        with infer_graph.as_default(), tf.container('infer'):
            infer_iter = get_iterator("data/skyline3k-100k-2d.txt", params, is_infer=True)
            true = []
            logits = []
            points = []
            prob = []
                ...
                for i in range(params.num_gpus):
                    with tf.device('/gpu:%d' % i):
                        with tf.name_scope('tower_%d' % (i)) as scope:
                            model = SkyNet_Layer(params, 'infer', infer_iter.get_next())
