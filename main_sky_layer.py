...
from skyNet_flayer import SkyNet_Layer
...
def preprocess(x):
    src_trgt_layer = tf.compat.v1.string_split([x], delimiter="outputly").values  # seperate the src data and output set
    src, trgt, layer = src_trgt_layer[0], src_trgt_layer[1], src_trgt_layer[2]
    ...
    layer = tf.compat.v1.string_to_number(tf.compat.v1.string_split([layer]).values, tf.float32)
    return src, tf.shape(src)[0], trgt, tf.shape(trgt)[0], layer


def infer_preprocess(x):
...
def get_iterator(filename, params, is_infer=False):
    ...
    if is_infer:
        dataset = dataset.map(infer_preprocess,
                              num_parallel_calls=8)  # preprocess the dataset and put it in the iterator
        ...
        dataset = dataset.padded_batch(params.batch_size,
                                       padded_shapes=([None, params.input_dim], [], [None], [], [None]))
...
def logits_to_index(logits):
    ...
    for i in range(m):
        ...
        temp_idx = np.where(temp_logits > 0.5)[0].tolist()
...
if __name__ == '__main__':
    ...
    if params.mode == 'train':
        ...
        if len(sys.argv) >= 2:
            ...
            train_file = "data/skyline10k-10to1k-2d-inde-layer-train.txt"
            infer_file = "data/skyline10k-10to1k-2d-inde-layer-train.txt"

        with train_graph.as_default(), tf.compat.v1.container("train"):
            train_iter = get_iterator(train_file, params)
            train_model, train_op, loss = build_multi_gpu_model(params, SkyNet_Layer, train_iter)
        ...
        with eval_graph.as_default(), tf.compat.v1.container("eval"):
            ...
            eval_model, op, eval_loss = build_multi_gpu_model(params, SkyNet_Layer, eval_iter)
        ...
        if len(sys.argv) >= 2:
            ...
            infer_file = "data/skyline10k-10to1k-2d-inde-train.txt"

        with infer_graph.as_default(), tf.compat.v1.container('infer'):
            infer_iter = get_iterator(infer_file, params, is_infer=True)
            infer_model = SkyNet_Layer(params, params.mode, infer_iter.get_next())
        ...
        while True:
            try:
                ...
                pred = logits_to_index(result['logits'])
                ...
                for i in range(len(result['true'])):
                    ...
                    tf.compat.v1.logging.info("true:      {}".format(sorted(list(true_set))))
                    print()
                    tf.compat.v1.logging.info("predicted: {}".format(sorted(list(pred_set))))
                    print("=======================================================")
                ...
                tf.compat.v1.logging.info(s1 + '\n' + s2 + '\n' + s3 + '\n' + s4)
                tf.compat.v1.logging.info("time elapsed: {}".format(time_elapsed))
                ...
                print(total_count)
                print("total time elapsed: {}".format(total_time.microseconds / total_count))
                print(
