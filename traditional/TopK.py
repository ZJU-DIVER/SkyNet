...
def get_top_n_items(array, n):
    """ Get the top n items. """
    nth = select_random_pivot(array, n)  # avg case: O(N), WC: O(N^2)
    top_items = []

    # Start by getting all of the values less than the top value
    # (O(N)). This guarantees that we don't miss some because there
    # are duplicate nth values.
    for value in array:
        if value < nth:
